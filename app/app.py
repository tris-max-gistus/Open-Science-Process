...
ARTIFACT_CATEGORIES = ("helpful", "problematic", "personal_value")
...
<body>
    ...
    <script>
        ...
        function renderArtifactForm() {
            ...
            const categories = [['helpful', 'Helpful'], ['problematic', 'Problematic'], ['personal_value', 'Personal value']];

            let html = `
                <h2>Save an AI Output</h2>
                <div class="modal-subtext">This is the one place in this tool where it's OK to paste something the AI produced. Use it to flag output worth keeping.</div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label>How would you categorize it?</label>
                    <div class="scale-group">
                        ${categories.map(([val, text]) => `
                            <div class="scale-option">
                                <input type="radio" id="artifact_cat_${val}" name="artifact_category" value="${val}">
                                <label for="artifact_cat_${val}">${text}</label>
                            </div>
                        `).join('')}
                    </div>
                </div>
        ...
        async function submitArtifact() {
            ...
            const category = document.querySelector('input[name="artifact_category"]:checked')?.value || '';
...
def add_artifact():
    """Save a flagged AI response as a standalone .txt file. No JSON log involvement."""
    if not SESSION_STATE["active_promptlog"]:
        return jsonify({"status": "error", "message": "No active prompt log"}), 400
    ...
    filename = f"artifact_{stamp}_{category}.txt"
    filepath = artifact_dir / filename
    suffix = 2
    while filepath.exists():
        filename = f"artifact_{stamp}_{category}_{suffix}.txt"
        filepath = artifact_dir / filename
        suffix += 1
